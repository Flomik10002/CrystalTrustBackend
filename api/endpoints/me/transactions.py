...
#
# MONTHS_NOMINATIVE = ["", "января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября",
...
#             select(User.nickname).where(User.id == sa.owner_id)
#             .scalar_subquery().label("from_nick"),
#             select(User.nickname).where(User.id == ta.owner_id)
#             .scalar_subquery().label("to_nick"),
#         )
...
#
#     def push(dt: datetime, amount: int, row, type_: str, from_nick: str, to_nick: str, from_acc: str, to_acc: str,
#              avatar: str):
...
#         grouped[key].append({
#             "id": f"tx{row.id}",
#             "name": to_nick if amount > 0 else from_nick,
#             "amount": amount,
#             "type": type_,
#             "from": from_nick,
...
#
#         if is_deposit:
#             push(r.created_at, r.amount, r, "Пополнение",
...
#
#         if is_withdraw:
#             push(r.created_at, -r.amount, r, "Снятие",
...
#                 amount = +r.amount
#
#             push(r.created_at, amount, r, "Перевод",
#                  r.from_nick, r.to_nick,
#                  fmt_acc(r.source_public), fmt_acc(r.target_public),
...
async def get_transactions(
    ...
    for r in rows:
        ...
        if is_deposit:
            # push(
            #     r.created_at, r.amount, r, "Пополнение",
            #     a_settings.BANK_NICK, r.to_nick,
            #     "", fmt_acc(r.target_public),
            #     a_settings.BANK_AVATAR
            # )
        ...
        if is_withdraw:
            # push(
            #     r.created_at, -r.amount, r, "Снятие",
            #     r.from_nick, a_settings.BANK_NICK,
            #     fmt_acc(r.source_public), "",
            #     a_settings.BANK_AVATAR
            # )
